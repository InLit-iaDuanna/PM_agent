...
from pm_agent_worker.tools.minimax_settings import (
    MiniMaxConnectionSettings,
    MiniMaxSettings,
    is_placeholder_api_key,
    load_minimax_settings,
)


class MiniMaxChatClient:
    def __init__(self, settings: Optional[MiniMaxSettings] = None) -> None:
        self.settings = settings or load_minimax_settings()
        ...
        if not self._has_usable_connection():
            self._config_error = "MINIMAX_API_KEY 未配置，或仍是占位值"
    ...
    def _ordered_connections(self) -> List[tuple[int, MiniMaxConnectionSettings]]:
    ...
    def _mark_connection_succeeded(self, connection_index: int, connection: MiniMaxConnectionSettings) -> None:
    ...
    def complete(self, messages: List[Dict[str, str]], temperature: float = 0.2, max_tokens: int = 1800) -> str:
        if not self.is_enabled():
            if self._config_error:
                raise RuntimeError(f"MiniMax 客户端不可用：{self._config_error}")
        ...
        for connection_index, connection in self._ordered_connections():
            ...
            try:
                with httpx.Client(timeout=self._build_timeout(connection.timeout_seconds)) as client:
                    response = client.post(connection.chat_completions_url, headers=headers, json=payload)
                ...
                last_error = error
                self._mark_connection_failed(connection_index, error)
            except (KeyError, IndexError, TypeError) as error:
                last_error = error
                continue

        if isinstance(last_error, (KeyError, IndexError, TypeError)):
            self._last_error = "返回结构异常"
            raise RuntimeError(f"MiniMax 返回结构异常：{json.dumps(last_response_data or {}, ensure_ascii=False)[:400]}") from last_error
        self._last_error = self._error_text(last_error or RuntimeError("未知请求错误"))
        raise RuntimeError(f"MiniMax 请求失败：{self._last_error}") from last_error
